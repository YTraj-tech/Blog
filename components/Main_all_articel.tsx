...
const MainAllArticles = () => {
  ...
  if (loading) {
    return (
      <div className="relative min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 overflow-hidden">
        <div className="absolute inset-0 bg-[linear-gradient(rgba(255,255,255,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.03)_1px,transparent_1px)] bg-[size:50px_50px] [mask-image:radial-gradient(ellipse_at_center,black_70%,transparent_100%)]"></div>
        <div className="absolute inset-0 overflow-hidden">
          <div className="absolute top-1/4 left-1/4 w-24 h-24 sm:w-48 sm:h-48 md:w-96 md:h-96 bg-purple-500/10 rounded-full mix-blend-multiply filter blur-3xl animate-pulse"></div>
          <div className="absolute top-3/4 right-1/4 w-24 h-24 sm:w-48 sm:h-48 md:w-96 md:h-96 bg-blue-500/10 rounded-full mix-blend-multiply filter blur-3xl animate-pulse delay-1000"></div>
          <div className="absolute bottom-1/4 left-1/2 w-24 h-24 sm:w-48 sm:h-48 md:w-96 md:h-96 bg-indigo-500/10 rounded-full mix-blend-multiply filter blur-3xl animate-pulse delay-2000"></div>
        </div>
        <div className="relative z-10 grid grid-cols-1 gap-y-6 px-4 py-6 pt-16 sm:pt-20">
          {[1, 2, 3].map((i) => (
            <div key={i} className="animate-pulse mx-auto w-full max-w-[350px] sm:max-w-[450px]">
              <div className="bg-white/10 backdrop-blur-sm rounded-xl h-80 sm:h-96 border border-white/20"></div>
            </div>
          ))}
        </div>
      </div>
    ...
    return (
      <div className="relative min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 overflow-hidden flex items-center justify-center">
        ...
        <motion.p
          ...
          animate={{ opacity: 1 }}
          transition={{ duration: 0.5 }}
          className="relative z-10 text-lg sm:text-xl text-white/90 text-center p-6"
          style={{ fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif" }}
        >
          {error}
        </motion.p>
    ...
    >
      ...
      <div className="relative z-10 p-4 sm:p-6 pt-16 sm:pt-20">
        ...
        >
          <div className="flex justify-center sm:justify-start mb-4">
            <Link href="/">
              <Button
                variant="outline"
                size="sm"
                className="flex border-white/30 text-white hover:border-white/50 hover:bg-white/10 transition-colors rounded-lg backdrop-blur-sm px-3 py-1 sm:px-4 sm:py-2"
              >
                <Home className="h-4 w-4 mr-2" />
                Back to Home
              </Button>
          ...
          >
            Explore Our
            <span className="bg-gradient-to-r from-blue-400 via-purple-400 to-pink-400 bg-clip-text text-transparent ml-2 sm:ml-3">
              Blog
            </span>
          </h1>
          <p
            className="text-base sm:text-lg text-white/90 mt-2 font-medium"
            style={{ fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif" }}
          >
            Discover insightful articles, tips, and stories from our community
          </p>
        ...
        >
          <div className="relative">
            <Search className="absolute left-3 top-2.5 sm:top-3.5 h-4 w-4 sm:h-5 sm:w-5 text-white/60" />
            <Input
              type="text"
              placeholder="Search articles by title..."
              value={searchValue}
              onChange={(e) => setSearchValue(e.target.value)}
              onKeyDown={handleSearchKeyDown}
              className="pl-9 sm:pl-10 py-2 sm:py-3 rounded-lg border border-white/20 focus:ring-2 focus:ring-blue-400 bg-white/10 backdrop-blur-sm shadow-sm text-white placeholder-white/60 text-sm sm:text-base"
            />
          </div>
        </motion.div>
        {pagination && !searchValue && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            transition={{ duration: 0.4 }}
            className="text-center text-xs sm:text-sm text-white/70 mb-6"
            style={{ fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif" }}
          >
            Showing {articles.length} of {pagination.totalArticles} blog posts
          </motion.div>
        ...
        {filteredArticles.length === 0 ? (
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-6 px-2 sm:px-4 md:px-6 lg:px-8 py-6 place-items-center">
              <AnimatePresence>
                {filteredArticles.map((article) => (
                  <motion.div
                    key={article.id}
                  ...
                  >
                    <Card
                      className="group cursor-pointer bg-white/95 backdrop-blur-xl rounded-xl shadow-2xl hover:shadow-purple-500/25 transition-all duration-300 overflow-hidden border border-white/20 hover:scale-[1.02] sm:hover:scale-105"
                      onClick={() => router.push(`/articels/${article.id}`)}
                    >
                      <div className="absolute top-3 sm:top-4 left-3 sm:left-4 z-10">
                        <span className="bg-gradient-to-r from-purple-600 to-pink-600 text-white px-2 sm:px-3 py-1 rounded-full text-xs font-medium flex items-center gap-1 shadow-lg">
                          <TrendingUp className="w-3 h-3" />
                          Trending
                        </span>
                      </div>
                      {article.featuredImage && (
                        <div className="relative w-full h-48 sm:h-56 bg-gradient-to-br from-purple-200 via-blue-200 to-pink-200 overflow-hidden">
                          <Image
                            src={article.featuredImage}
                            alt={article.title}
                            fill
                      ...
                      <div className="p-4 sm:p-6 space-y-3 sm:space-y-4">
                        <div className="flex items-center gap-2">
                          <div className={`w-2 h-2 rounded-full ${getCategoryColor(article.category)}`}></div>
                          <span
                            className="text-xs sm:text-sm font-medium text-gray-600 capitalize"
                            style={{ fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif" }}
                          >
                            {article.category}
                          </span>
                        ...
                        <CardHeader className="p-0">
                          <CardTitle
                            className="text-lg sm:text-xl font-bold text-gray-900 line-clamp-2 leading-tight group-hover:text-purple-600 transition-colors duration-200"
                            style={{ fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif" }}
                          >
                            {article.title}
                          </CardTitle>
                        ...
                        <CardContent className="p-0">
                          <p
                            className="text-gray-600 text-sm sm:text-base line-clamp-3 leading-relaxed"
                            style={{ fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif" }}
                          >
                            {article.content}
                          </p>
                        ...
                        <div className="flex items-center gap-3 pt-2">
                          <div className="w-7 sm:w-8 h-7 sm:h-8 rounded-full bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center overflow-hidden shadow-lg">
                            {article.author.imageUrl ? (
                              <Image
                                src={article.author.imageUrl}
                                alt={article.author.name}
                      ...
                      <CardFooter className="px-4 sm:px-6 pb-4 sm:pb-6 pt-0">
                        <div className="w-full space-y-3 sm:space-y-4">
                          ...
                          <Button
                            className="w-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-bold py-2 sm:py-3 rounded-xl transition-all duration-200 shadow-lg hover:shadow-xl transform hover:scale-105"
                            onClick={(e) => {
                              e.stopPropagation();
                              router.push(`/articels/${article.id}`);
                            }}
                            style={{ fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif" }}
                          >
                            Read More
            ...
            {pagination && !searchValue && (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                transition={{ duration: 0.4 }}
                className="flex flex-wrap justify-center items-center mt-6 sm:mt-8 space-x-2 gap-y-2"
              >
                <Button
                  variant="outline"
                  size="sm"
                  onClick={handlePrevious}
                  disabled={!pagination.hasPreviousPage}
                  className="flex items-center border-white/30 text-white hover:border-white/50 hover:bg-white/10 transition-colors rounded-lg backdrop-blur-sm px-3 py-1 sm:px-4 sm:py-2"
                  style={{ fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif" }}
                >
                  <ChevronLeft className="h-4 w-4 mr-1" />
                  Previous
                </Button>
                <div className="flex items-center flex-wrap justify-center space-x-1 gap-y-2">
                  {Array.from({ length: pagination.totalPages }, (_, i) => i + 1).map((page) => (
                    <Button
                      key={page}
                      variant={page === currentPage ? 'default' : 'outline'}
                      size="sm"
                      onClick={() => handlePageChange(page)}
                      className={`min-w-[36px] sm:min-w-[40px] rounded-lg transition-all duration-200 ${
                        page === currentPage
                          ? 'bg-gradient-to-r from-purple-600 to-pink-600 text-white hover:from-purple-700 hover:to-pink-700 shadow-lg'
                          : 'border-white/30 text-white hover:bg-white/10 hover:border-white/50 backdrop-blur-sm'
                      }`}
  ...
  export default MainAllArticles;